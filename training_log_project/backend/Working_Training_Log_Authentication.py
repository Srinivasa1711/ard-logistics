...
SMTP_USERNAME = 'bhicks@ardlogistics.com'  # Replace with admin email
SMTP_PASSWORD = 'Hicks406!'          # Replace with email password
...
server_name = 'ARDKPISRV-19\\SQLEXPRESS'
...
def sync_employees_from_api(access_token):
    ...
    try:
        ...
        # --- FIX: Extract employees from 'records' key ---
        if isinstance(api_response, dict) and "records" in api_response:
            employees = api_response["records"]
        ...
        for emp_data in employees:
            ...
            user_account_status = str(emp_data.get("userAccountStatus", "")).strip().lower()

            print(f"[DEBUG] Considering employee_id: {employee_id}, employmentStatus: {employment_status}, userAccountStatus: {user_account_status}")
    ...
    # Count employees with 'Active' status
            if employment_status == 'active' or user_account_status == 'active':
...
def start_scheduler():
    ...
    scheduler.add_job(scheduled_refresh_and_sync, 'interval', minutes=30)
    scheduler.start()
